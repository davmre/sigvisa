...
from learn import load_earth, load_netvisa
...
def main(param_dirname):
  ...
  parser.add_option("-s", "--sited", dest="siteid",
                    default=6, help = "site to display",
                    metavar="RUNID", type="int")
...
def EventDetection(options, earthmodel, netmodel, start_time, end_time,
  ...
  for plot_phaseid in range(numtimedefphases):
    ...
    y_pts = [np.exp(netmodel.detection_logprob(1, 0, 3.5, x, options.siteid,
                                                 plot_phaseid)) \
             for x in x_pts]
