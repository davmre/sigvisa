...
class StateSpaceModel(object):
    ...
    def kalman_predict(self, k, xk1, Pk1, xk, Pk):
        ...
        self.apply_transition_matrix(xk1, k, xk)
    ...
    def kalman_observe(self, k, zk, xk, Pk, KkT):
    ...
    def run_filter(self, z, likelihood_only=True):
        ...
        xk = self.prior_mean()
        Pk = np.diag(self.prior_vars())

        tmp = np.empty((self.max_dimension,))

        ell = self.kalman_observe(0, z[0], xk, Pk, tmp)
        ...
        Pk1 = Pk.copy()
        for k in range(1, N):
            self.kalman_predict(k, xk1, Pk1, xk, Pk)
            ell += self.kalman_observe(k, z[k], xk, Pk, tmp)
    ...
    def prior_sample(self, N):
        x = scipy.stats.multivariate_normal(self.prior_mean(), np.diag(self.prior_vars())).rvs(1)
        ...
        pred_state = x.copy()
        for k in range(1, N):
            self.apply_transition_matrix(x, k, pred_state)
            self.transition_bias(k, pred_state)

            noise_diag = np.empty((len(pred_state),))
            self.transition_noise_diag(k, noise_diag)
            noise_cov = np.diag(noise_diag)
            x = scipy.stats.multivariate_normal(pred_state, noise_cov).rvs(1)
