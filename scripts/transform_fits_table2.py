...
for row in cursor:
    ...
    if not key in fitids:
        insert_query = "insert into sigvisa_coda_fit (runid, evid, sta, chan, band, optim_method, iid, stime, etime, acost, dist, azi) values (%d, %d, '%s', '%s', '%s', '%s', %d, %f, %f, %f, %f, %f)" % (
            row[0], row[1], row[2], row[3], band, row[11], row[12], row[13], row[14], row[15], row[16], row[17])
    ...
    phase_insert_query = "insert into sigvisa_coda_fit_phase (fitid, phase, template_model, arrival_time, peak_offset, coda_height, coda_decay) values (%d, '%s', 'paired_exp', %f, %f, %f, %f)" % (
        fitids[key], row[6], row[7], row[8], row[9], row[10])
