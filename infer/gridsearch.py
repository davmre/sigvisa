...
def main():
    ...
    parser.add_option("--max_evtime_proposals", dest="max_evtime_proposals", default=5, type="int", help="maximum number of event times to consider per gridsearch location")
    parser.add_option("--use_true_depth", dest="use_true_depth", default=False, action="store_true", help="use the true depth of the event being searched for (default is to integrate over depth)")
    ...
    tm = load_template_model(template_shape = options.template_shape, model_type=model_types, run_name=run_name, run_iter=run_iter, sites=sites)
...
#    wm = StupidL1WiggleModel(tm)
    ...
    if options.use_true_depth:
        f = lambda lon, lat: ev_loc_ll_at_optimal_time(event_at(ev_true, lon=lon, lat=lat), segments, log_likelihood=f_ll, template_model=tm, phases=phases, max_proposals = options.max_evtime_proposals)
    else:
        f = lambda lon, lat: integrate_ll_over_depth(event_at(ev_true, lon=lon, lat=lat), segments=segments, log_likelihood=f_ll, template_model=tm, phases=phases, max_proposals = options.max_evtime_proposals)
