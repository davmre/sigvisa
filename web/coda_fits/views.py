...
def FitImageView(request, pk):
    ...
    fit = SigvisaCodaFit.objects.get(fitid=pk)
    s = Sigvisa()

    band_name = s.band_name(low_match = fit.lowband, high_match=fit.highband)
    (phases, vals), cost = load_template_params(fit.evid, fit.sta, fit.chan, band_name, fit.run_name, fit.run_iter)

    seg = load_event_station(fit.evid, fit.sta, cursor=s.cursor).with_filter("env;"+band)
    wave = seg[fit.chan]
