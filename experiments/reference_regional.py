...
def relevant_events(region):
    ...
    sql_query = "select evid from leb_origin where (time between %.1f and %.1f) and not ((lat between %.1f and %.1f) and (lon between %.1f and %.1f) ) " % (relevant_stime, relevant_etime, region.bottom_lat, region.top_lat, region.left_lon, region.right_lon)
    print sql_query
...
def reference_events(region, bulletin="isc"):
    relevant_stime = region.stime - MAX_TRAVEL_TIME
    relevant_etime = region.etime
...
def main(hour=0.0, len_hours=2.0, runid=37, hz=2.0, tmpl_steps=500, ev_steps=1000, resume_from=None, deserialize=None, uatemplate_rate=4e-4, raw_signals=False, bands=["freq_0.8_4.5"], fix_outside=True, phases=("P"), target_evid=-1, stime=None, etime=None, reference_bulletin="isc"):
    ...
    evs = reference_events(region, reference_bulletin)
    for ev in evs:
        evnodes = sg.add_event(ev)
        eid = evnodes["lon"].eid
