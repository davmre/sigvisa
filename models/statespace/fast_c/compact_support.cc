...
CompactSupportSSM::CompactSupportSSM(\
       std::vector<int> & start_idxs, std::vector<int> & end_idxs,
  ...
  int i;
  std::vector<int>::const_iterator st;
  std::vector<int>::const_iterator et;
  for( i = 0, st = this->start_idxs.begin(), et = this->end_idxs.begin() ;
    ...
    for (int j= std::max(*st, 0); j < std::min(*et, this->n_steps); ++j) {
      ...
      (this->active_indices)[std::make_pair(i,j)] = int(tmp_active_basis[j].size());
    }
  }
...
int CompactSupportSSM::apply_transition_matrix(const vector<double> &x, int k, vector<double> &result){
  ...
  for (i=0, idx = active.begin();
    ...
    dense_hash_map<std::pair<int,int>, int>::iterator contains = this->active_indices.find(key);
    if (contains == this->active_indices.end()) {
      result(i) = 0;
    } else {
      double prev_idx = this->active_indices[key];
      result(i) =x(prev_idx);
    }
  }
  return i;
...
double CompactSupportSSM::apply_observation_matrix(const vector<double> &x, int k) {
  ...
  for (i=0, idx = active.begin();
    ...
    int prototype = this->identities[*idx];
    int st = this->start_idxs[*idx];
