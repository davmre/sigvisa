...
PyObject * py_arr_likelihood(SigModel_t * p_sigmodel, PyObject * args) {
  ...
  double best_score = p_model->likelihood(p_sigmodel, p_segment, 1, (const Arrival_t **)&p_best);
  ...
  for (double amp = min_amp; amp <= max_amp; amp *= amp_step) { 
    p_arr->amp = amp;
    double score = p_model->likelihood(p_sigmodel, p_segment, 1, (const Arrival_t **)&p_arr);
    if (score > best_score) {
      best_score = score;
  ...
  memcpy(p_arr, p_best, sizeof(Arrival_t));
    ...
    pred_segment->siteid = p_segment->siteid;
    pred_segment->len = p_segment->len;
    p_model->sample(p_sigmodel, 
		       p_sigmodel->p_earth,
		       pred_segment,
		       1, (const Arrival_t **)&p_best,
		       0, 0);
